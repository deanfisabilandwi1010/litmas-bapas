...
<head>
  ...
  if ($_SESSION['status'] == 'login') { ?>
    ...
    <title>BKA CB (Pegawai) - Bapas Pekanbaru</title>
...
<li class="nav-item">
  ...
  <main id="main" class="main">
    ...
    <a class="btn btn-primary" href="../pegawai/bka-datakliencb.php" style="float: right;" role="button">+</a>
    <div>
      <table class="table" id="table">
        ...
        <tbody>
          ...
          $result2 = pg_query(
              $conn,
              'SELECT nama_pegawai FROM pegawai INNER JOIN litmas ON pegawai.nip = litmas.nip WHERE id_jenis_litmas = 1 AND id_jenis_klien= 1 ORDER BY id_litmas ASC'
          );
          $result3 = pg_query(
              $conn,
              'SELECT nama_lapas FROM lapas INNER JOIN litmas ON lapas.id_lapas = litmas.id_lapas WHERE id_jenis_litmas = 1 AND id_jenis_klien= 1 ORDER BY id_litmas ASC'
          );
          $result4 = pg_query(
              $conn,
              'SELECT jenis_kasus FROM kasus INNER JOIN litmas ON kasus.id_kasus = litmas.id_kasus WHERE id_jenis_litmas = 1 AND id_jenis_klien= 1 ORDER BY id_litmas ASC'
          );
          $result5 = pg_query(
              $conn,
              'SELECT nama_status_litmas FROM status_litmas INNER JOIN litmas ON status_litmas.id_status = litmas.id_status WHERE id_jenis_litmas = 1 AND id_jenis_klien= 1 ORDER BY id_litmas ASC'
          );
          ...
          while ($row = pg_fetch_array($result)) {
            ...
            <tr align="center">
              ...
              <td>
                ...
                <div class="con">
                  <a class="bi bi-pencil-square bg-icon-primary " href="../pegawai/bka-statuscb.php?id_litmas=<?= $row[
                      'id_litmas'
